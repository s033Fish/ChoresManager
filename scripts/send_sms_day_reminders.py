...
def get_user_chores():
    """
    Fetch chores grouped by user for the current day.
...
def send_reminders():
    ...
    if not user_chores:
        print("No uncompleted chores found for today.")
        return
    ...
    for (user_id, first_name, last_name, phone_number), chores in user_chores.items():
        ...
        message = (
            f"Hi {first_name}, here are your pending chores for today:\n"
            f"{chore_list}\n"
            "Please complete them as soon as possible. Thank you!"
        )
        send_sms(phone_number, message)
        users_contacted.append(f"{first_name} {last_name} ({phone_number}) {chore_list}")
        print(f"Sent reminder to {first_name} {last_name} at {phone_number} for {chore_list}.")
